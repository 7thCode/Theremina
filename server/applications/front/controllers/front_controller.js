        ...
        build(request, response) {
            ...
            let copy = (resourcename) => {
                let copy_resource = (resolve, reject) => {
                    ResourceModel.findOne({ $and: [{ namespace: namesapce }, { userid: config.systems.userid }, { name: resourcename.original }, { type: 30 }] }, {}, {}).then((doc) => {
                        if (doc) {
                            let name = resourcename.target;
                            let content = doc.content;
                            ResourceModel.findOne({ $and: [{ namesapce: namesapce }, { userid: userid }, { name: name }, { type: 20 }] }).then((found) => {
                                if (!found) {
                                    let page = new ResourceModel();
                                    page.userid = userid;
